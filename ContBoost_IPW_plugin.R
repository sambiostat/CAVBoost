...
MyBoost_cont_IPWplugin<-function(dat, otr.test, dat.test, PredVar ){
  ...
  fit = MyBoost_cont_sub_IPW_plugin(dat)
...
MyBoost_cont_sub_IPW_plugin <- function(dat){
  N <- nrow(dat)
  labels <- dat$trt01p
  ...
  W[W == 0|is.na(W)] = min(W[W != 0], na.rm =T)/1.1
  
  indweight = rep(0,N)
  indweight[labels==1] =   1/W[labels==1]
  indweight[labels==0] =   1/(1-W[labels==0])
