...
MyBoost_cont<-function(dat, otr.test, dat.test, PredVar ){
...
MyBoost_cont_sub <- function(dat){
  N <- nrow(dat)
  labels <- dat$trt01p
  dtrain <- xgb.DMatrix(as.matrix(subset(dat, select=-c(trt01p, Y))), label = labels)
  
  #attach other info to the dtrain data
  attr(dtrain, "dcopy") = as.matrix(subset(dat, select=-c(trt01p, Y)))  
  attr(dtrain, "Y") = dat$Y
  ...
  evalerror <- function(preds, dtrain) {
    ...
    dcopy = attr(dtrain, "dcopy")
    Y = attr(dtrain, "Y")
    #prop = attr(dtrain, "prop")[1]
    #K = attr(dtrain, "K")[1]
    #print(paste0("EVAL : ", prop ))
    arm.val <- c(1,0)
